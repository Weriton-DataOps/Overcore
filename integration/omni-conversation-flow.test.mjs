...
test('cliente produtivo Omni responde ao Preflight e recebe resultado da mesma tarefa real', { timeout: 180_000 }, async t => {
  ...
  try {
    ...
    const runtime = { async run(request, sink, signal) {
      ...
      runtimeCalls.push({ purpose: request.purpose, authSource: result.authSource, model: result.model, permissionDenials: result.permissionDenials })
      return result
    } }
    ...
    const nextInput = { reportId: first.reportId, answers: first.decisions.map(decision => ({ decisionId: decision.decisionId,
      optionId: decision.options.find(option => option.label === 'Não produzir artefato')?.optionId || decision.recommendedOptionId })), changes: {
    ...
    evidence = { flowId: first.flowId, taskId: observed.taskId, statuses: [first.status, scheduled.status, observed.status], questions: first.decisions.length, runtimeCalls }
    success = true
  } finally {
